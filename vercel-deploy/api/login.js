...
// Cache em memória para instâncias serverless
...
export default async function handler(req, res) {
  ...
  const u = String(username || '').replace(/\0/g, '').trim().toLowerCase();
  const p = String(password || '').replace(/\0/g, '').trim();
  ...
  // 3. Falha: Registra tentativa e calcula lockout
  ...
  if (attempts.length >= MAX_ATTEMPTS) {
    ...
    res.setHeader('Retry-After', LOCKOUT_MS / 1000);
    return res.status(429).json({
